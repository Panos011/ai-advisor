...
3c. Optionally send 'last_shown_tool' — the name of the single tool the app most recently rendered. Pronoun follow-ups ("is it free?", "why that one?", "the last one") resolve against the server's last-shown memory, which a deploy wipes; passing 'last_shown_tool' restores the referent so those questions answer about the right card instead of silently falling back to the first one. Surviving server memory wins; absent it, behavior is unchanged.
